# Exercise 14: Orders Operations Console (Completion)
...
That means the generated client at `src/api/client/` does **not** export the helpers your solution needs. Open `src/challenges/8-hour/solutions/exercise-14-end.tsx` right now — TypeScript will scream about ~20 errors. The starter on the other hand only uses `getApiV1OrdersOptions` and compiles fine.
